...
function pickBookTop(book) {
  ...
  return { bidCount:bids.length, askCount:asks.length, bestBidRaw:bids[0]??null, bestAskRaw:asks[0]??null };
}
function isNativeRolling(row) {
  const asset = String(row.asset ?? '').toUpperCase();
  const interval = Number(row.intervalSec ?? 0);
  return (asset === 'BTC' || asset === 'ETH') && (interval === 900 || interval === 3600);
}
...
async function main() {
  ...
  const nativeRows = live.filter(isNativeRolling);
  const poolOfInterest = (nativeRows.length ? nativeRows : live)
    .slice()
    .sort((a,b) => (Number(b.expiry ?? 0) - now) - (Number(a.expiry ?? 0) - now));

  const inspected = [];
  let chosen = null;
  for (const row of poolOfInterest.slice(0, 12)) {
    ...
    try {
      ...
      inspected.push({marketId, pool, secondsLeft, nativeRolling:isNativeRolling(row), readError:String(error?.message??error)});
      continue;
    }
    const entry = {
      marketId, pool, asset:row.asset??null, intervalSec:Number(row.intervalSec??0), expiry:Number(row.expiry??0), secondsLeft,
      nativeRolling:isNativeRolling(row), onchainStatus:Number(onchain.status),
    };
    ...
    try {
      ...
      chosen = {
        ...entry,
        onchain:serial(onchain), bookParams:serial(params), bookTop:serial(pickBookTop(rawBook)),
        collateral:{address:SOMNIA_TESTNET_ADDRESSES.collateral,decimals:6,symbol:'tUSDC'},
      };
    } catch (error) {
  ...
  const result = {
    schema:'LKB-SHANNON-PREFLIGHT-v0.4', generatedAt:new Date().toISOString(), mode:'READ_ONLY_NO_SIGNER_LOW_LEVEL_BOUNDED',
    chainId:50312, sdkPinnedVersion:'0.29.0', indexerUrl:INDEXER_URL, minHeadroomSec:MIN_HEADROOM_SEC,
    liveMarketCount:live.length, nativeRollingMarketCount:nativeRows.length, chosen, inspected,
    authority:{blockchainWritePerformed:false,privateKeyPresent:false,nextBoundary:chosen?'PREPARE_WRITE_PACKET_ONLY':'BLOCK_NO_SAFE_FULL_READ'},
  };
...
main().catch((error)=>{console.error('LKB_SHANNON_PREFLIGHT_FAILED',error);process.exit(1);});
